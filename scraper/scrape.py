...
def extract_positions_from_tables(tables: list, full_text: str) -> list:
    ...
    # State Machine (Durum Takibi) Değişkenleri
    current_faculty = ""
    current_dept = ""
    pending_titles_with_counts = []
    pending_reqs = []
    
    # Mevcut birikmiş (bekleyen) kadroyu ve şartlarını kaydeden yardımcı fonksiyon
    def save_pending_block():
        if not pending_titles_with_counts:
            return
        # Şart satırlarını aralarında boşlukla birleştir (String Concatenation)
        req_text = " ".join(r for r in pending_reqs if r).strip()
        
        for title, count in pending_titles_with_counts:
            pos = {
            ...
            # Her bir kadro için Özel Şartlar metninden ALES ve Dil çek
            ales = extract_ales(req_text, title)
            if not ales["alesRequired"]: ales = extract_ales(full_text, title)
            lang = extract_language(req_text, title)
            if not lang["foreignLanguageRequired"]: lang = extract_language(full_text, title)
            pos.update(ales)
            pos.update(lang)
            positions.append(pos)
        
        # Kaydettikten sonra sepeti (hafızayı) temizle
        pending_titles_with_counts.clear()
        pending_reqs.clear()
    ...
    for table in tables:
        ...
        for row in table[start_idx:]:
            ...
            # Sütunlardan verileri çek
            fac_val = row[current_col["faculty"]] if "faculty" in current_col else ""
            dept_val = row[current_col["dept"]] if "dept" in current_col else ""
            title_val = row[current_col["title"]] if "title" in current_col else ""
            count_val = row[current_col["count"]] if "count" in current_col else "1"
            req_val = row[current_col["req"]] if "req" in current_col else ""
            
            # 1. HAFIZAYI GÜNCELLE (Forward Fill)
            # Eğer satırda dolu bir fakülte veya bölüm varsa hafızayı ona güncelle.
            if fac_val: current_faculty = fac_val
            if dept_val: current_dept = dept_val
            
            # 2. YENİ KADRO TETİKLEYİCİSİ (Sadece "Unvan" sütununda aranır!)
            row_titles = extract_titles_from_cell(title_val)
            
            if row_titles:
                # EĞER YENİ UNVAN BULUNDUYSA: Bu yeni bir kadrodur!
                # a) Eskisini paketle ve kaydet
                save_pending_block()
                
                # b) Yeni kadronun Adetlerini hesapla
                counts = [int(c) for c in re.findall(r'\d+', str(count_val))]
                if not counts: counts = [1]
                
                # c) Yeni unvanları (ve adetleri) hafızaya al
                for idx, t in enumerate(row_titles):
                    c = counts[idx] if idx < len(counts) else counts[-1]
                    pending_titles_with_counts.append((t, c))
                    
                # d) Yeni kadronun özel şartlarını sepete at
                if req_val:
                    pending_reqs = [req_val]
            else:
                # EĞER UNVAN YOKSA: Bu bir devam (continuation) satırıdır!
                # Sadece özel şart cümlesini mevcut sepete eklemeye devam et.
                if req_val:
                    pending_reqs.append(req_val)
                    
    # Tüm tablolar ve satırlar bittikten sonra hafızada kalan en son kadroyu da kaydet
    save_pending_block()
